...
def local2global(semanticId, instanceId):
    ...
    if isinstance(globalId, np.ndarray):       # check if globalId is the the type of np.ndarray
...
class KITTI360Bbox3D(KITTI360Object):
    ...
    def parseVertices(self, child):
        ...
        vertices = np.matmul(R, vertices.transpose()).transpose() + T  # coordinates under world coordinate system
...
class Annotation3D:
    ...
    def init_instance(self, labelPath):
        # load annotation
        tree = ET.parse(labelPath)  # analyse the xml file
